 ...
 * Author3: Felice Paparusso <felice.paparusso@gmail.com>
 * Author4: Vincenzo Fortunato <vincfort92@hotmail.it>
 */
...
Interference::ComputeInterference (UserEquipment *ue, bool MBSFNConstructiveInterference)
{
  ...
  for (auto node : *NetworkManager::Init ()->GetENodeBContainer ())
    {
...
//cout << "selecting interference from cell " << node->GetIDNetworkNode() << endl;
              ...
              if (idCell == idCellPrec)
              ...
              else
                {
                  ...
                  switch(channelModel)
                    ...
                    // created for each pair of ue-enb
  ...
  for (auto henb : *NetworkManager::Init ()->GetHomeENodeBContainer())
    {
...
double Interference::ComputeDopplerInterference (int speed /* in km/h */, LtePhy::WaveformType type)
{
  vector< pair<double, double> >* interferenceTable;
  if (type==LtePhy::WAVEFORM_TYPE_OFDM)
    {
      interferenceTable = &m_OFDMInterferenceValues;
    }
  else if (type==LtePhy::WAVEFORM_TYPE_POFDM)
    {
      interferenceTable = &m_POFDMInterferenceValues;
    }
  else if (type==LtePhy::WAVEFORM_TYPE_IDEAL_NO_DOPPLER)
    {
      return INFINITY;
    }
  else
