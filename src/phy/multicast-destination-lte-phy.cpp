...
#include <iostream>
#include <algorithm>
#include "multicast-destination-lte-phy.h"
...
MulticastDestinationLtePhy::MulticastDestinationLtePhy()
{
  Simulator::Init()->Schedule(0.001,&MulticastDestinationLtePhy::CreateCqiFeedbacks,this);
}

void
MulticastDestinationLtePhy::CreateCqiFeedbacks ()
{
...
//      if(enb==GetDevice()->GetTargetNode() && (int)temp_cqi.size() > 0)
      vector<ENodeB*> sources = GetDevice()->GetSources ();
      if ( find(
             sources.begin(),
             sources.end (),
             enb
...
MulticastDestinationLtePhy::GetDevice(void)
{
