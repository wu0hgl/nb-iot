...
#include "../../protocolStack/mac/random-access/ue-enhanced-lte-random-access.h"
...
Application::Start ()
{
DEBUG_LOG_START_1(LTE_SIM_TEST_START_APPLICATION)
...
DEBUG_LOG_END
        ...
        {
          for (auto enb:GetTransmittingNodes())
            {
              //create an DL radio bearer between targetENB and UE
              UserEquipment* ue = (UserEquipment*) GetDestination ();
              ue->MakeActive();
...
DEBUG_LOG_END
  ...
  if (GetSource ()->GetNodeType() == NetworkNode::TYPE_UE)
           ...
           || GetSource ()->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION)
    {
      ...
      LteChannel *mcch = ue->GetTargetNode ()->GetPhy ()->GetDlMcChannel ();
