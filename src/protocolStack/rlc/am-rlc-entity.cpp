...
DEBUG_LOG_START_1(LTE_SIM_RLC_DEBUG)
  cout  << "InsertAMDIntoReceptionList" << endl;
  PrintReceivedAMDs ();
DEBUG_LOG_END
...
DEBUG_LOG_START_1(LTE_SIM_RLC_DEBUG)
  cout << "AM RLC tx procedure for node " << GetRadioBearerInstance ()->GetSource ()->GetIDNetworkNode ()<< " bearer "<<  GetRlcEntityIndex () << endl;
DEBUG_LOG_END
...
DEBUG_LOG_START_1(LTE_SIM_RLC_DEBUG)
              cout << "_____ pkt " << packet->GetID() << " frag " << packet->GetRLCHeader ()->GetFragmentNumber () <<
                        " sn " << packet->GetRLCHeader ()->GetRlcPduSequenceNumber () <<
                        " startB " << packet->GetRLCHeader ()->GetStartByte () <<
                        " endB " << packet->GetRLCHeader ()->GetEndByte () << endl;
              PrintSentAMDs ();
...
{
DEBUG_LOG_START_1(LTE_SIM_RLC_DEBUG)
  cout << "AM RLC rx procedure for node " << GetRadioBearerInstance ()->GetDestination ()->GetIDNetworkNode () << " bearer "<<  GetRlcEntityIndex () << endl;
  cout << "_____ pkt " << p->GetID() << " frag " << p->GetRLCHeader ()->GetFragmentNumber () <<
  ...
  for (auto amdRecord : *m_receivedAMDs)
    {
      ...
      if (packet->GetID () != currentPacket && packet->GetRLCHeader ()->GetStartByte () == 0)
        ...
        {
          //cout << "\t\t !! fragment of a new packet, but not he first" << endl;
          expectedNextByte = 0;
        }
...
DEBUG_LOG_START_1(LTE_SIM_RLC_DEBUG)
  cout << "\t\t !! DELETE REASSEBLED PACKETS" << endl;
DEBUG_LOG_END
