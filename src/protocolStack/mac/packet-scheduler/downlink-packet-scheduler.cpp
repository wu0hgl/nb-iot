...
void DownlinkPacketScheduler::SetMaxUsersPerRB(int users) {
	m_maxUsersPerRB = users;
}

int DownlinkPacketScheduler::GetMaxUsersPerRB(void) {
...
void DownlinkPacketScheduler::SelectFlowsToSchedule() {
	...
	for (auto bearer : *rrc->GetRadioBearerContainer()) {
		...
		if (bearer->HasPackets()
				&& bearer->GetDestination()->GetNodeState()
						== NetworkNode::STATE_ACTIVE
				&& (FrameManager::Init()->MbsfnEnabled() == false
						|| (FrameManager::Init()->MbsfnEnabled() == true
								&& FrameManager::Init()->isMbsfnSubframe()
										== true
								&& bearer->GetDestination()->GetNodeType()
										== NetworkNode::TYPE_MULTICAST_DESTINATION)
						|| (FrameManager::Init()->MbsfnEnabled() == true
								&& FrameManager::Init()->isMbsfnSubframe()
										== false
								&& bearer->GetDestination()->GetNodeType()
			...
			ENodeB *enb = GetMacEntity()->GetDevice();
			...
			AMCModule* amc =
					ueRecord->GetUE()->GetProtocolStack()->GetMacEntity()->GetAmcModule();
			...
			for (int i = 0; i < numberOfCqi; i++) {
				...
				if (txMode == 11 && false) {
					...
					for (int n = 0; n < (int) fullCsiFeedbacks.at(i)->n_rows;
							n++) {
					...
					int nbOfRBs =
							GetMacEntity()->GetDevice()->GetPhy()->GetBandwidthManager()->GetDlSubChannels().size();
					shared_ptr<arma::cx_fmat> local_precoding =
							PrecodingCalculator::RegularizedZeroForcing(
									channelMatrix1, ranks, widebandSinrs,
									pow(10.,
											(GetMacEntity()->GetDevice()->GetPhy()->GetTxPower()
													- 30) / 10) / nbOfRBs);
					double avgPower = arma::mean(
							(arma::fvec) arma::sum(
									arma::square(
											arma::abs(*fullCsiFeedbacks.at(i))),
									1));
					double noise_interference = 10 * log10(avgPower)
							- 2 * amc->GetSinrFromCQI(cqiFeedbacks.at(i));
//cout << "SINR_TEST2 ue "
...
//    << endl;
					vector<double> sinrs_cqi = SinrCalculator::MimoReception(
							*fullCsiFeedbacks.at(i), *local_precoding,
							noise_interference, allLayers);
...
//<< endl;
				fill(spectralEfficiency.begin(), spectralEfficiency.end(),
						sum_se / count_se);
			}
			...
			if (std::getenv("USE_COVERSHIFT") != nullptr) {
				...
				CartesianCoordinates* src_position =
						enb->GetMobilityModel()->GetAbsolutePosition();
				CartesianCoordinates* dst_position =
						ue->GetMobilityModel()->GetAbsolutePosition();
				double array_bearing =
						enb->GetPhy()->GetAntennaParameters()->GetBearing()
								* M_PI / 180.0;

				// UE horizontal angle compared to antenna boresight
				double dst_angle_h = dst_position->GetPolarAzimut(src_position)
						- array_bearing;
				if (dst_angle_h > M_PI) {
					dst_angle_h -= (2 * M_PI);
				}
				...
				if (src->GetAntennaParameters()->GetBearing() == 0) {
					if (index == 0) {
						grid_angle_h_min = grid_angle_h_min
								+ sector_width * 0.32;
						grid_angle_h_max = grid_angle_h_max
								- sector_width * 0.32;
					} else if (index == 1) {
						grid_angle_h_min = grid_angle_h_min
								+ sector_width * 0.65;
				...
				if (dst_angle_h < grid_angle_h_min
						|| dst_angle_h > grid_angle_h_max) {
					continue;
				}
			}
			...
			InsertFlowToSchedule(bearer, dataToTransmit, spectralEfficiency,
					cqiFeedbacks, riFeedback, pmiFeedbacks, fullCsiFeedbacks);
		} else {
		}
	}
}
	...
	for (auto flow : *GetFlowsToSchedule()) {
		...
		if (availableBytes > 0) {
			...
			if (flow->IsHarqRetransmission() == false) {
				...
				if (flow->GetBearer()->GetDestination()->GetNodeType()
						== NetworkNode::TYPE_MULTICAST_DESTINATION) {
...
void DownlinkPacketScheduler::RBsAllocation() {
	...
	ENodeB::UserEquipmentRecords* ueRecords =
			GetMacEntity()->GetDevice()->GetUserEquipmentRecords();
	for (auto ue : *ueRecords) {
		HarqManager* harqManager = ue->GetHarqManager();
		if (harqManager != nullptr) {
			FlowsToSchedule harqFlows =
			...
			flowsForRetransmission.insert(flowsForRetransmission.end(),
					harqFlows.begin(), harqFlows.end());
		}
	}
	...
	for (int i = 0; i < nbOfRBs; i++) {
		for (int j = 0; j < (int) flows->size(); j++) {
			...
			ENodeB::UserEquipmentRecord* ueRecord =
					flows->at(j)->GetUe()->GetTargetNodeRecord();
			...
			if (harqPid == HarqManager::HARQ_NO_PID_AVAILABLE
					&& flows->at(j)->IsHarqRetransmission() == false) {
	...
	for (int i = 0; i < nbOfRBs; i++) {
		for (int j = 0; j < (int) flowsForRetransmission.size(); j++) {
			HARQ_metrics[i][j] = ComputeSchedulingMetric(
					flowsForRetransmission.at(j)->GetBearer(),
	...
	DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG)
		cout << ", available RBs " << nbOfRBs << ", flows " << flows->size()
				<< endl;
	...
	int l_iNumberOfUsers =
			GetMacEntity()->GetDevice()->GetNbOfUserEquipmentRecords();
	...
	for (auto harqFlow : flowsForRetransmission) {
		...
		iota(schedulingMetricsIndices.begin(), schedulingMetricsIndices.end(),
				0);

		sort(schedulingMetricsIndices.begin(), schedulingMetricsIndices.end(),
		...
		if (availableRBs >= numRequiredRBs && numRequiredRBs > 0) {
			HarqScheduledUEs.push_back(
					flowsForRetransmission.at(flowIndex)->GetUe()->GetIDNetworkNode());
			int assignedRBs = 0;
			for (int i = 0; i < nbOfRBs; i++) {
				if (allocationsPerRB.at(schedulingMetricsIndices.at(i))
						< GetMaxUsersPerRB()) {
					allocatedRBs->push_back(schedulingMetricsIndices.at(i));
					allocationsPerRB.at(schedulingMetricsIndices.at(i))++;assignedRBs
					++;
				}
			...
			flowsForRetransmission_copy.erase(
					remove(flowsForRetransmission_copy.begin(),
							flowsForRetransmission_copy.end(), harqFlow));
		}
	}
	...
	for (int rb = 0; rb < nbOfRBs; rb++) {
		...
		for (int i = allocationsPerRB.at(rb); i < m_maxUsersPerRB; i++) {
			...
			for (int k = 0; k < (int) flows->size(); k++) {
				if (metrics[rb][k] > targetMetric && metrics[rb][k] > -INFINITY
						&& flows->at(k)->GetUe()->GetTargetNodeRecord()->GetHarqManager()->GetPidForNewTx()
								>= 0 && !l_bFlowScheduled[k]
						&& flows->at(k)->IsHarqRetransmission() == false
						&& find(HarqScheduledUEs.begin(),
								HarqScheduledUEs.end(),
								flows->at(k)->GetUe()->GetIDNetworkNode())
								== HarqScheduledUEs.end()
						&& find(scheduledFlows.begin(), scheduledFlows.end(),
								flows->at(k)) == scheduledFlows.end()) {
			...
			if (targetMetric > 0) {
				allocationsPerRB.at(rb)++;scheduledFlows
				.push_back(scheduledFlow);
				l_iScheduledFlowsIndices.push_back(l_iScheduledFlowIndex);
			}
		}
		DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG)
			cout << "Number of flows on RB " << rb << ": "
					<< scheduledFlows.size() << endl;
		DEBUG_LOG_END
		...
		if (RBIsAllocated) {
			...
			if (txMode == 11) {
				if (precodingMatrices.at(rb) == nullptr) {
					...
					vector<int> ranks;
					int layerCounter = 0;
					vector<shared_ptr<FlowToSchedule> > allScheduledFlows =
							scheduledFlows;
					allScheduledFlows.insert(allScheduledFlows.end(),
							flowsForRetransmission.begin(),
							flowsForRetransmission.end());
					for (int k = 0; k < (int) allScheduledFlows.size(); k++) {
						shared_ptr<FlowToSchedule> flow = allScheduledFlows.at(
								k);
						// TODO: choose layers here or in specialized scheduler?
						vector<int> assignedLayers;
						if (flow->GetFullCsiFeedbacks().size() > 0) {
							channelMatrices.push_back(
									flow->GetFullCsiFeedbacks().at(rb));
					...
					precodingMatrices.at(rb) =
							PrecodingCalculator::RegularizedZeroForcing(
									channelMatrices, ranks, widebandSinrs,
									pow(10.,
											(GetMacEntity()->GetDevice()->GetPhy()->GetTxPower()
													- 30) / 10)
											/ (double) nbOfRBs);
		...
		if (RBIsAllocated) {
			...
			for (int i = 0; i < (int) scheduledFlows.size(); i++) {
				...
				AMCModule* amc =
						scheduledFlow->GetUe()->GetProtocolStack()->GetMacEntity()->GetAmcModule();
				scheduledFlow->GetListOfAllocatedRBs()->push_back(rb); // the rb RB has been allocated to that flow!
				int flowIndex = distance(flows->begin(),
						find(flows->begin(), flows->end(), scheduledFlow));
				...
				DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG)
					cout << "\t *** RB " << rb << " assigned to the "
							...
							<< " (UE "
							<< scheduledFlow->GetUe()->GetIDNetworkNode()
							<< " rank " << scheduledFlow->GetRiFeedback() << ")"
							<< endl;
				DEBUG_LOG_END
				double sinr = amc->GetSinrFromCQI(
						scheduledFlow->GetCqiFeedbacks().at(rb));
				l_bFlowScheduledSINR[l_iScheduledFlowsIndices.at(i)].push_back(
						sinr);
				int mcs = amc->GetMCSFromSinrVector(
						l_bFlowScheduledSINR[l_iScheduledFlowsIndices.at(i)]);
				int txMode =
						scheduledFlow->GetUe()->GetTargetNodeRecord()->GetDlTxMode();
				int M, N, rank;
				M =
						scheduledFlow->GetBearer()->GetSource()->GetPhy()->GetTxAntennas();
				N =
						scheduledFlow->GetBearer()->GetDestination()->GetPhy()->GetRxAntennas();
				switch (txMode) {
				case 1:
				case 2:
					rank = 1;
				...
				case 11:
					...
					if (rb == nbOfRBs - 1) {
						int id =
								scheduledFlows.at(i)->GetUe()->GetIDNetworkNode();
						vector<double> newSinr =
								PrecodingCalculator::adjustSinrForMuMimo(
										l_bFlowScheduledSINR[l_iScheduledFlowsIndices.at(
												i)],
										scheduledFlow->GetUeRecord()->GetChannelMatrix(),
										precodingMatrices,
										scheduledFlow->GetAssignedLayers());
						mcs = amc->GetMCSFromSinrVector(newSinr);

						DEBUG_LOG_START_1(LTE_SIM_MCS_DEBUG)
							cout << "BS_SINR_2 UE "
									<< scheduledFlows.at(i)->GetUe()->GetIDNetworkNode()
									<< " SINR " << newSinr << " MCS " << mcs
									<< endl;
						DEBUG_LOG_END
					}
					break;
				default:
					cout << "Error: invalid transmission mode " << txMode
							<< endl;
					break;
				}
				int transportBlockSize = amc->GetTBSizeFromMCS(mcs, mcs,
						scheduledFlow->GetListOfAllocatedRBs()->size(), rank);
				if (transportBlockSize
						>= scheduledFlow->GetDataToTransmit() * 8) {
	...
	flows->insert(flows->end(), flowsForRetransmission.begin(),
			flowsForRetransmission.end());
	...
	for (auto flow : *flows) {
		...
		if (flow->GetListOfAllocatedRBs()->size() > 0) {
			...
			if (flow->IsHarqRetransmission() == false) {
				...
				for (int rb = 0;
						rb < (int) flow->GetListOfAllocatedRBs()->size();
						rb++) {
					double sinr;
					sinr = amc->GetSinrFromCQI(
							flow->GetCqiFeedbacks().at(
				...
				if (FrameManager::Init()->isMbsfnSubframe() == true) {
					MulticastDestination* mc =
							(MulticastDestination*) flow->GetBearer()->GetApplication()->GetDestination();
					...
					if (mc_mcs == -1) {
						...
						cout << "Wrong MCS set for multicast stream: " << mc_mcs
								<< endl;
						exit(1);
					}
...
//                  delete amc;
					...
					DEBUG_LOG_START_1(LTE_SIM_MCS_DEBUG)
						cout << "MCS_DEBUG estimated SINR values: UE "
								<< flow->GetUe()->GetIDNetworkNode()
								<< ", SINRs " << estimatedSinrValues << endl;
					DEBUG_LOG_END
					vector<double> newSinr =
							PrecodingCalculator::adjustSinrForMuMimo(
									estimatedSinrValues,
									...
									precodingMatrices,
									flow->GetAssignedLayers());
//                  mcs = amc->GetMCSFromSinrVector(newSinr);
					mcs = GetMacEntity()->GetAmcModule()->GetMCSFromSinrVector(
							newSinr);

					DEBUG_LOG_START_1(LTE_SIM_MCS_DEBUG)
						cout << "MCS_DEBUG adjusted SINR values: UE "
								<< flow->GetUe()->GetIDNetworkNode()
								<< " SINRs " << newSinr << endl;
					DEBUG_LOG_END
//                  mcs = amc->GetMCSFromSinrVector(estimatedSinrValues);
			...
			case 3:
				if (rank < 1 || rank > min(M, N)) {
					cout << "Error: invalid rank in DownlinkPacketScheduler ("
							<< rank << ")" << endl;
					rank = min(M, N);
				}
				pmi.resize(0); //not used
				DEBUG_LOG_START_1(LTE_SIM_TEST_RI_FEEDBACK)
					cout << "flow "
							<< flow->GetBearer()->GetApplication()->GetApplicationID()
							<< ",\tRI feedback: " << flow->GetRiFeedback()
							<< endl;
				DEBUG_LOG_END
				break;
			...
			if (flow->IsHarqRetransmission() == false) {
				...
				int transportBlockSize = amc->GetTBSizeFromMCS(mcs, mcs,
						flow->GetListOfAllocatedRBs()->size(), rank);
			...
			if (txMode != 11) {
				...
				for (int rb = 0;
					...
					if (flow->GetBearer()->GetDestination()->GetNodeType()
						...
						MulticastDestination* virtualDest =
								(MulticastDestination*) flow->GetBearer()->GetDestination();
						for (auto dest : virtualDest->GetDestinations()) {
							ENodeB::UserEquipmentRecord* userRecord =
									dest->GetTargetNodeRecord();
							if (userRecord != nullptr) {
								HarqManager* harq =
										userRecord->GetHarqManager();
								int harqPID;
								if (harq != nullptr) {
									harqPID =
											userRecord->GetHarqManager()->GetPidForNewTx();
								} else {
									harqPID = HarqManager::HARQ_NOT_USED;
								}
				...
				for (int rb = 0;
					...
					if (flow->GetBearer()->GetDestination()->GetNodeType()
						...
						pdcchMsg->AddNewRecord(
								...
								flow->GetBearer()->GetDestination(),
								flow->GetListOfSelectedMCS()->at(rb),
								flow->GetHarqPid(), rank, precodingMatrix,
								assignedLayers,
								flow->GetFullCsiFeedbacks().at(
										flow->GetListOfAllocatedRBs()->at(rb)));
					} else {
						pdcchMsg->AddNewRecord(
								PdcchMapIdealControlMessage::DOWNLINK,
...
//                          RadioBearer* rbearer;
//                          int data;
							shared_ptr<FlowToSchedule> flowcp =
									make_shared < FlowToSchedule
											> (flow->GetBearer(), flow->GetDataToTransmit());
							*flowcp = *flow;
				...
				ueRecord->GetHarqManager()->NewTransmission(flow->GetHarqPid(),
						flow);
			}
		}
	}
