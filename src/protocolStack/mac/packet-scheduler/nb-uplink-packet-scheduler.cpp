...
nbUplinkPacketScheduler::nbUplinkPacketScheduler(EnbMacEntity* mac)
{
  SetMacEntity (mac);
  CreateUsersToSchedule ();
...
{
  CreateUsersToSchedule ();
  ENodeB *node = GetMacEntity ()->GetDevice ();

DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG_VERBOSE)
...
DEBUG_LOG_END
          ...
          ENodeB::UserEquipmentRecord* record = user->m_userToSchedule->GetTargetNodeRecord();
//          cout << "\t UE " << user->m_userToSchedule->GetIDNetworkNode() <<
//        		  " mcs " <<  user->m_selectedMCS << " translate: " << user->m_transmittedData <<
//        		  " T " << Simulator::Init()->Now() << endl;											// by zyb
