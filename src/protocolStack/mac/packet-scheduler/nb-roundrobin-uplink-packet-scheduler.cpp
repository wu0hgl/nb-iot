...
nbRoundRobinUplinkPacketScheduler::nbRoundRobinUplinkPacketScheduler(
	...
	for (int i = 0;
			i
					< mac->GetDevice()->GetPhy()->GetBandwidthManager()->GetUlSubChannels().size();
			i++) {
...
void nbRoundRobinUplinkPacketScheduler::RUsAllocation() {
	int currentSF = FrameManager::Init()->GetTTICounter();
	int ttiLength = FrameManager::Init()->getTTILength();
	EnbNbIoTRandomAccess* enbRam =
			(EnbNbIoTRandomAccess*) GetMacEntity()->GetRandomAccessManager();

	UsersToSchedule *users = GetUsersToSchedule();

	int ruSlice = 10;

	if (currentSF % ttiLength == 0 && !(enbRam->isRachOpportunity())) {
		DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG_LOG)
			cout << "LOG_USERS TTI " << currentSF << " UE " << users->size()
					<< endl;
		DEBUG_LOG_END
		int freeTones = count(m_RUmap[0].begin(), m_RUmap[0].end(), -1);

		DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG_RR)
			cout << "SF: " << currentSF
					<< " -> NB RR UL RUs Allocation - Users: " << users->size();
		...
		int tones =
				GetMacEntity()->GetDevice()->GetPhy()->GetBandwidthManager()->GetTones();
		...
		if (qSize == 0) {
			...
			for (auto scheduledUser : *users) {
				...
				vector<UserToSchedule>::iterator i = find(m_queue.begin(),
						m_queue.end(), *scheduledUser);
		...
		while (s < qSize) {
			...
			if (freeTones > 0) {
				...
				id = scheduledUser.m_userToSchedule->GetIDNetworkNode();
				userAlreadyScheduled = find(m_RUmap[0].begin(),
						m_RUmap[0].end(), id) != m_RUmap[0].end();

				if (!userAlreadyScheduled) {
					int dataToTransmit = scheduledUser.m_dataToTransmit;
					CartesianCoordinates* userPosition =
							scheduledUser.m_userToSchedule->GetMobilityModel()->GetAbsolutePosition();
					CartesianCoordinates* cellPosition =
							scheduledUser.m_userToSchedule->GetCell()->GetCellCenterPosition();
					double cellRadius =
							scheduledUser.m_userToSchedule->GetCell()->GetRadius();
					double distance = userPosition->GetDistance(cellPosition);
					int mcs =
							GetMacEntity()->GetNbAmcModule()->GetMCSfromDistance(
									distance, cellRadius * 1000, tones);

					DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG_RR)
						cout << " \t Assigning resources to user " << id
								<< endl;
						cout << " \t User Distance = " << distance << endl;
						cout << " \t Data to transmit [byte] = "
								<< dataToTransmit << endl;
						cout << " \t MCS = " << mcs;
					DEBUG_LOG_END

					int nru =
							GetMacEntity()->GetNbAmcModule()->GetNbOfRUsFromSize(
									mcs, dataToTransmit * 8);
					...
					int tbs =
							(GetMacEntity()->GetNbAmcModule()->GetTBSizeFromMCS(
									mcs, nru)) / 8;

					DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG_RR)
						cout << " - TBS [byte] = " << tbs << " - NRU = " << nru
								<< endl;
					DEBUG_LOG_END

					vector<int>::iterator i = find(m_RUmap[0].begin(),
							m_RUmap[0].end(), -1); //find first free subcarrier
					...
					DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG_RR)
						cout << "\t User "
								...
								<< " has been already scheduled." << endl;
					DEBUG_LOG_END
				}
				m_roundRobinId++;
			}
		...
		DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG_LOG)
			cout << "LOG_RESOURCES TTI " << currentSF << " SUBCARRIERS "
					<< m_RUmap[1].size()
							- std::count(m_RUmap[0].begin(), m_RUmap[0].end(),
									-1) << endl;
		DEBUG_LOG_END
		DEBUG_LOG_START_1(LTE_SIM_SCHEDULER_DEBUG_RR_MAP)
			printMap();
		DEBUG_LOG_END
		...
		for (int i = 0; i < m_RUmap[1].size(); i++) {
			if (m_RUmap[1][i] > 0) {
				if (--m_RUmap[1][i] <= 0) {
					//if (m_RUmap[5][i] > 0) {
						//cout << "SRC " << m_RUmap[0][i] << " repeatID " << m_RUmap[5][i] << endl;
						//m_RUmap[1][i] = m_RUmap[4][i];
						//if (--m_RUmap[5][i] <= 0) {

							id = m_users[i].m_userToSchedule->GetIDNetworkNode();
							if (m_users[i].m_dataToTransmit - m_RUmap[3][i] <= 0) {
								vector<UserToSchedule>::iterator j = find(
										m_queue.begin(), m_queue.end(),
										m_users[i]);
								vector<UserToSchedule>::size_type idx = j
										- m_queue.begin();
								m_queue.erase(j);
							...
							for (auto user : *users) {
								if (user->m_userToSchedule->GetIDNetworkNode()
										== id) {
									user->m_transmittedData = m_RUmap[3][i];
									user->m_selectedMCS = m_RUmap[2][i];
									user->m_listOfAllocatedRUs.push_back(
											m_RUmap[4][i]);
									user->m_subcarrier = i;
								}
							}
