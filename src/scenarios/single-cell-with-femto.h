  ...
  for (int i = nbCell; i < nbCell + nbFemtoCells; i++)
    {
      ...
      if (accessPolicy == 1)
        {
          enb->GetMacEntity()->SetRestrictedAccessMode(false);
        }
      ...
      for (int i = 0; i < nbUE; i++)
        {
          ...
          for (int k = 0; k < nbCell; k++)
            {
              ChannelRealization* c_dl = new ChannelRealization (eNBs->at (k), ue, ChannelRealization::CHANNEL_MODEL_MACROCELL_URBAN);
              eNBs->at (k)->GetPhy ()->GetDlChannel ()->GetPropagationLossModel ()->AddChannelRealization (c_dl);
              ChannelRealization* c_ul = new ChannelRealization (ue, eNBs->at (k), ChannelRealization::CHANNEL_MODEL_MACROCELL_URBAN);
              eNBs->at (k)->GetPhy ()->GetUlChannel ()->GetPropagationLossModel ()->AddChannelRealization (c_ul);
            }
