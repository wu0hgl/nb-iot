...
static double
ComputePathLossForInterference (ENodeB* src, UserEquipment* dst, bool isLosType)
{
  /*
  ...
  if (src->GetNodeType() == NetworkNode::TYPE_ENODEB && dst->GetNodeType () == NetworkNode::TYPE_UE)
    {
      ...
      if( src->GetPhy ()->GetAntennaParameters ()->GetType() == LtePhy::AntennaParameters::ANTENNA_TYPE_TRI_SECTOR )
        {
