...
NetworkManager::NetworkManager()
{
  ...
  m_eNodeBContainer = new vector<ENodeB*>;
  m_homeENodeBContainer = new vector<HeNodeB*>;
                              ...
                              BandwidthManager *bm)
{
  ...
  enb->GetPhy ()->SetTxAntennas (numTxAntennas);
  enb->GetPhy ()->SetRxAntennas (numRxAntennas);
  enb->GetPhy ()->SetDlChannel (dlCh);
//  XXX enb->GetPhy ()->SetDlMcChannel (dlMcCh);
                                     ...
                                     Cell* cell, ENodeB* enb)
{
  ...
  ue->GetPhy ()->SetDlMcChannel (enb->GetPhy()->GetDlMcChannel ());
  ...
  if (enb->GetPhy()->GetDlMcChannel () != nullptr)
    {
      ChannelRealization* c_mcdl = new ChannelRealization (enb, ue, ChannelRealization::CHANNEL_MODEL_MACROCELL_URBAN, true);
...
{
  for (auto eNodeB : *GetENodeBContainer ())
    {
...
bool
NetworkManager::CheckHandoverPermissions (ENodeB* target, UserEquipment* user)
{
...
void
NetworkManager::HandoverProcedure(double time, UserEquipment* ue, ENodeB* oldTarget, ENodeB* newTarget)
{
...
DEBUG_LOG_START_2(LTE_SIM_HANDOVER_DEBUG,LTE_SIM_HANDOVER_TARGET_DEBUG)
  cout << "** HO ** \t HandoverProcedure for user "
            <<  ue->GetIDNetworkNode ()
            << " from eNB " << oldTarget->GetIDNetworkNode()
            << " to eNB " << newTarget->GetIDNetworkNode() << endl;
DEBUG_LOG_END
...
DEBUG_LOG_START_1(LTE_SIM_MOBILITY_DEBUG)
  cout << "user " <<  ue->GetIDNetworkNode () << " starts HO "
            ...
            "\n\t target eNB = " <<  newTarget->GetIDNetworkNode () << endl;
DEBUG_LOG_END
...
void
NetworkManager::TransferBearerInfo (UserEquipment* ue, ENodeB* target)
{
...
DEBUG_LOG_END
  ...
  if ( (target->GetNodeType() == NetworkNode::TYPE_ENODEB && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_ENODEB) ||
    ...
    {
      ENodeB *oldTargetNode = ue->GetTargetNode ();
      ENodeB *newTargetNode = target;
...
DEBUG_LOG_END
      ...
      LteChannel *oldDl, *oldUl, *oldMcDl, *newDl, *newUl, *newMcDl;
...
DEBUG_LOG_END
      ...
      // --> move dl bearers
DEBUG_LOG_START_1(LTE_SIM_HANDOVER_DEBUG)
      cout << "\t DL radio bearers " << oldEnbRrc->GetRadioBearerContainer ()->size () << endl;
DEBUG_LOG_END
...
DEBUG_LOG_END
      ...
      for (auto bearer : *oldEnbRrc->GetRadioBearerSinkContainer ())
        {
...
{
  cout << "UEsForCell: ";
  for (auto record : *GetENodeBContainer())
    {
...
NetworkManager::Print (void)
{
  ...
  for (auto enb : *GetENodeBContainer())
    {
      enb->Print();
    }
...
NetworkManager::PrintFrequencyMask (void)
{
  for (auto eNodeB : *GetENodeBContainer())
    {
      cout << "ENODEB " <<  eNodeB->GetIDNetworkNode() << ", frequencyMask:" << endl;
    }
}
