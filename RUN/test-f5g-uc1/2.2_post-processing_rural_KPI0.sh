...
for speed in ${speed_values}; do
	...
	echo "Calculating user experienced data rate with speed = ${speed} km/h (rural scenario)"
	echo "density \"speed ${speed} km/h\"" > "${temp_file}"

	for density in ${density_values}; do
		for file in "${data_dir}/sim_uc1_rural_density_${density}ue_km2_${speed}kmh_seed"* ; do
			gunzip -c  "${file}" \
				| awk "
					/^PHY_RX/{
						bits[\$5] += \$17;
