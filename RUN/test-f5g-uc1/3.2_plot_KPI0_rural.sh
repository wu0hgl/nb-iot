...
PLOT_SIZE=1600,1200
KPI0_graph="${results_dir}/KPI0_rural.png"
col_count=$(echo $speed_values | awk '{print NF+1}')
xrange="[40:110]"
yrange="[0:70]"
target_y=50

echo "Plotting user experienced data rate (rural scenario)"
...
echo "set title 'User experienced data rate (rural scenario)'" >> "${GNUPLOT_FILE}"
